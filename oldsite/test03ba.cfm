<!DOCTYPE html>
<html>
<head>
<meta http-equiv="Content-Type" content="text/html; charset=utf-8" />
	<title>Long Beach City College Library</title>
	<meta name="description" content="" />
	<meta name="keywords" content="LBCC Library" />
	<meta http-equiv="Content-Type" content="text/html; charset=utf-8" />

<link rel="shortcut icon" href="" type="image/x-icon" />

    
<link href="http://www.lbcc.edu/favicon.ico" type="image/x-icon" rel="shortcut icon" />
<!--<link rel="stylesheet" href="jquery.ui.all.css" type="text/css" /> -->
<link type="text/css" rel="stylesheet" media="all" href="lbcclibrary.css" />	
<link rel="stylesheet" href="menu4.css" type="text/css" media="screen" />
<link rel="stylesheet" href="master.css" type="text/css" media="screen" />
<link rel="stylesheet" href="layout4.css" type="text/css" media="screen" />
<link rel="stylesheet" href="print.css" type="text/css" media="print" />
<link rel="stylesheet" href="front-page2.css" type="text/css" />
<link rel="stylesheet" href="search.css" type="text/css" />
<link rel="stylesheet" type="text/css" href="tabber.css" />
<link rel="stylesheet" type="text/css" href="http://api.libguides.com/css/libguides_api.css" />

<script src="jquery/hoverIntent.js" type="text/javascript" charset="utf-8"></script> 
<script src="jquery/script.js" type="text/javascript" charset="utf-8"></script>
<script src="http://ajax.googleapis.com/ajax/libs/jqueryui/1.8.4/jquery-ui.js" type="text/javascript" charset="utf-8"></script>

<script type="text/javascript"> 
$(document).ready(function () {

    
});

    

</script> 
<style>
    /* quick overrides */
    #libraryhours {background: #fff;}
    .quicklinks img {border:0px solid #fff; box-shadow:0px 0px 0px #fff;padding-bottom:80px;}
    tbody {border-collapse:collapse;border:0px #fff;border-spacing:0px;}
    body {background-image:url('images/new_index/blk.png'); background-repeat:repeat;}
    h1 {border-bottom:0px solid #ffffff}
    #footer p {color:#eee;}
    body {background-color:#090909;}

ul#mega li a {
color: #efefef!important;
    }
#nav_container {
background-color: #090909!important;
border-top: 2px solid #efefef!important;
border-bottom: 2px solid #efefef!important;
}
#top-content {
background: #fff!important;
}
#libraryhours {
background: #fff!important;
}
#footer-container {
background: #222!important;
}

#outer-head-container{
height: 80px!important;
}
    /* new stuff */
.bigbutton { height:80px;  
    }
.bigbutton h1 {margin-left:5px;line-height:130px;font-weight:100; font-size:1.2em;color:#090909;}
a:hover .bigbutton h1{color:#444444}
a:visited .bigbutton h1{color:#444444} 
.buttontext a:link,  p {font-size:.8em; line-height:1em; padding-right:10px;color:#090909;}
 .buttontext:hover {color:#444}  
 .buttontext:visited {color:#444} 
.first_buttontext {padding-left:10px;}
.mid_buttontext {margin-right:5px;}
.last_buttontext {margin-left:-10px;}
#books {/*border:1px dashed #090909; background-color:#fff;*/width:160px; background-image:url('images/new_index/book_gray_center.png'); background-repeat:no-repeat;margin-left:10px;}
/*a:hover #books {border:1px dashed #bbb;background-image:url('images/new_index/book_gray_light.png')}*/

#tbx {/*border:1px dashed #090909;background-color:#fff;*/width:110px; background-image:url('images/new_index/txbks_gray_center.png'); background-repeat:no-repeat; }
/*a:hover #tbx {border:1px dashed #bbb;background-image:url('images/new_index/txbks_gray_light.png')}*/
#art {/*border:1px dashed #090909;background-color:#fff;*/width:160px;background-image:url('images/new_index/art_gray_alt_center.png'); background-repeat:no-repeat;}
/*a:hover #art {border:1px dashed #bbb;background-image:url('images/new_index/art_gray_light_alt.png')}*/
#gdes {/*border:1px dashed #090909;background-color:#fff;*/width:160px;background-image:url('images/new_index/gdes_gray_center.png'); background-repeat:no-repeat;margin-left:-10px; }
/*a:hover #gdes {border:1px dashed #bbb;background-image:url('images/new_index/gdes_red.png')}*/
    </style>
</head>

<body>

<cfinclude template="topnav_buttons.cfm">
	

<div class="clear"> </div>
    
<div id="outer-content-container" class="container_16"> 
	<div id="content-container" class="container_16">  
			<div id="top-content">
                <p class="breadcrumb">&nbsp;</p>
                <cfinclude template="rightnavmain_logo.cfm">
				<div class="grid_11 alpha"> 
					 
<h1><i>Find</i></h1>

        <a href="http://lbcc.worldcat.org">
        <div class="grid_3">
            <div class="bigbutton" id="books"> <h1>Books, Movies&hellip;</h1>
             </div>                                       
             
        </div></a>
                                                   
        <a href="http://lbcc.worldcat.org/wcpa/courseReserves?action=courseReserveManager">
        <div class="grid_2">
            <div  class="bigbutton" id="tbx">    <h1>Textbooks</h1>                      
                
            </div>
            
        </div></a>
        
        <a href="http://lib.lbcc.edu/databases_subject.cfm">
        <div class="grid_3">
            <div class="bigbutton" id="art"> <h1>Articles & eBooks</h1>
             </div>                                       
             
        </div></a>
        
            
        
        <a href="http://lbcc.libguides.com">
            <div class="grid_3 omega">
                <div  class="bigbutton" id="gdes">   <h1>Study Guides</h1>                       
                    
                </div>
               
        </div></a>
 <p>&nbsp;</p>  <p>&nbsp;</p>          
                            
<div class="clear"> </div>


  <h1><i>Learn</i></h1>

        <a href="http://lbcc.worldcat.org">
        <div class="grid_3">
            <div class="bigbutton" id="books"> <h1>Tutorials</h1>
             </div>                                       
             
        </div></a>
                                                   
        <a href="http://lbcc.worldcat.org/wcpa/courseReserves?action=courseReserveManager">
        <div class="grid_2">
            <div  class="bigbutton" id="tbx">    <h1>Workshops</h1>                      
                
            </div>
           
        </div></a>
        
        <a href="http://lib.lbcc.edu/databases_subject.cfm">
        <div class="grid_3">
            <div class="bigbutton" id="art"> <h1>Orientations</h1>
             </div>                                       
            
        </div></a>
        
            
 <p>&nbsp;</p>  <p>&nbsp;</p>    <p>&nbsp;</p>  <p>&nbsp;</p>  <p>&nbsp;</p>         
   <p>&nbsp;</p>        
                            
<div class="clear"> </div>
<h1><i>Join</i></h1>

        <a href="http://lbcc.worldcat.org">
        <div class="grid_4">
            <div class="bigbutton" id="books"> <h1>Be a Library Tech</h1>
             </div>                                       
             
        </div></a>
                                                   
        <a href="http://lbcc.worldcat.org/wcpa/courseReserves?action=courseReserveManager">
        <div class="grid_3">
            <div  class="bigbutton" id="tbx">    <h1>Library Club</h1>                      
                
            </div>
           
        </div></a>
        
        <a href="http://lib.lbcc.edu/databases_subject.cfm">
        <div class="grid_3">
            <div class="bigbutton" id="art"> <h1>Work @Library</h1>
             </div>                                       
            
        </div></a>
        
                               
<div class="clear"> </div>
                  
 
</div> 


				</div>
			</div>
			<div class="clear"> </div>
			<div class="clear"> </div>
		
		<!-- </div>close outer-content-container -->

<div class="clear"></div>
<!-- close feature-container -->

<cfinclude template="footer.cfm">

</body>
</html>
